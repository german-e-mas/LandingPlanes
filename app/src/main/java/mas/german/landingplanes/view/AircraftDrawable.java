...
public abstract class AircraftDrawable extends Drawable {
  ...
  private void configurePath() {
    mPath.reset();
    mPath.moveTo(mX, mY - mRadius);
    mPath.lineTo(mX - mRadius * (float) Math.cos(Math.toRadians(30)),
        mY + mRadius * (float) Math.sin(Math.toRadians(30)));
    mPath.lineTo(mX, mY + 0.5f * mRadius * (float) Math.cos(Math.toRadians(60)));
    mPath.lineTo(mX + mRadius * (float) Math.cos(Math.toRadians(30)),
        mY + mRadius * (float) Math.sin(Math.toRadians(30)));
    mPath.close();
  }
  ...
  public void draw(Canvas canvas) {
    ...
    configurePath();
    canvas.rotate((float) Math.toDegrees(mDirection) + 90, mX, mY);
